...
/**
 * A blog post in Arabic.
 *
 * The slug stays English in every language — one post, one address per locale.
 * When the CMS has no Arabic translation ready the English text is served here
...
export async function generateMetadata({
  ...
  return buildPostMetadata(slug, 'ar');
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <BlogPostView slug={slug} locale="ar" />;
}
