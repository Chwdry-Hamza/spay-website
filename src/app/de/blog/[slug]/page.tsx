...
/**
 * A blog post in German.
 *
 * The slug stays English in every language — one post, one address per locale.
 * When the CMS has no German translation ready the English text is served here
...
export async function generateMetadata({
  ...
  return buildPostMetadata(slug, 'de');
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <BlogPostView slug={slug} locale="de" />;
}
