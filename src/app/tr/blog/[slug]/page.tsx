...
/**
 * A blog post in Turkish.
 *
 * The slug stays English in every language — one post, one address per locale.
 * When the CMS has no Turkish translation ready the English text is served here
...
export async function generateMetadata({
  ...
  return buildPostMetadata(slug, 'tr');
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return <BlogPostView slug={slug} locale="tr" />;
}
